...
export default function AdminLoginPage() {
  const [state, action, pending] = useActionState(
    async (_prev: { error?: string } | null, formData: FormData) => {
      return (await adminLogin(formData)) ?? null;
    },
    null
  );
  ...
  return (
    <div className="mx-auto flex min-h-full max-w-lg flex-col px-4 py-12">
      ...
      <div className="mb-8 text-center">
        ...
        <p className="mt-1 text-sm text-zinc-500">Masukkan PIN admin untuk melanjutkan</p>
      </div>
      ...
      <p className="mt-6 text-center text-xs text-zinc-400">
        Default PIN: 1234 (ubah via ADMIN_PIN di .env)
      </p>
    </div>
  );
}
